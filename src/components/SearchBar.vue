  ...
  if (newValue != "" || newValue != undefined) {
    ...
    if (searchValue.value !== "") {
      ...
      resultsTitle.value = searchArray.value.filter(
        (item) => item.postTitle.indexOf(searchValue.value) !== -1
      );
      resultsContent.value = searchArray.value.filter(
        (item) => item.postContent.indexOf(searchValue.value) !== -1
      );
      resultsAuthor.value = searchArray.value.filter(
        (item) => item.postAuthor.indexOf(searchValue.value) !== -1
      );

      resultsTitle.value.forEach((post) => {
      ...
      resultsContent.value.forEach((post) => {
        const whereFound = { whereFound2: "Content" };
        Object.assign(post, whereFound);
        const content = post;
        results.value.push(content);
      });
      resultsAuthor.value.forEach((post) => {
