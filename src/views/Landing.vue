...
async function fetchIamges() {
  ...
  const data = await bucket.media
    .find({
      folder: "front-page-carousel",
    })
    .props("imgix_url");
  frontImages.value = data.media;
}
  ...
  fetchIamges().then(() => {
    ...
    const observer = new IntersectionObserver(function (entries) {
      entries.forEach((entry) => {
        ...
        if (entry.target === section2 && entry.intersectionRatio === 1) {
          imageStart.value = true;
          imageGoing = setInterval(imageChange, 2500);
        } else {
          imageStart.value = false;
          clearInterval(imageGoing);
        }
      });
    }, options);
        ...
        <div class="images-wrapper">
          <transition-group name="carousel">
            <ul
              ...
              v-for="(image, index) in frontImages"
              :key="index"
            >
              <img
                @mouseover="hoveredImage"
                @mouseleave="unHoveredImage"
                class="image"
                :src="image.imgix_url"
                :alt="image"
                :key="index"
